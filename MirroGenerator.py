...
def tool():
    ...
    while True:
        ...
        z_old = - base_radius*base_radius / (4 * focal_length)
        z = - (base_radius - radius_increment_for_tool )* (base_radius - radius_increment_for_tool ) / (4 * focal_length)
        z_delta = z - z_old

        xy_delta = radius_increment_for_tool  # * base_radius / (diamiter/2)
        ...
        base_radius -= radius_increment_for_tool
#        if base_radius <= 0 :
        if base_radius <= radius_increment_for_tool :
            break
    
    #cut off the tool
    cut_off_radius = diamiter / 2 + bit_radius
    ...
    while True:
        ...
        print(  "G0 X-%.3f" %cut_off_radius,  " Y0 ", "Z%.3f" %z)
        print(  "G02 X-%.3f" %cut_off_radius,  " Y0 ", "I%.3f" %cut_off_radius, " J0 F%.1f" % feed_rate)
